...
/**
 * 字符串处理相关的类，对库中的一些字符串使用mb_string扩展实现
 * Class StringHelper
 * @package libs\Helper
 */
class StringHelper {
    ...
    public static function truncate($string, $length, $suffix = '...', $encoding = null, $asHtml = false)
    {
        ...
        if ($asHtml) {
            return self::truncateHtml($string, $length, $suffix, $encoding ?: self::$default_charset);
        }

        if (mb_strlen($string, $encoding ?: self::$default_charset) > $length) {
            return trim(mb_substr($string, 0, $length, $encoding ?: self::$default_charset)) . $suffix;
        } else {
            return $string;
        }
    }
    ...
    public static function startsWith($string, $with, $caseSensitive = true)
    {
        ...
        if ($caseSensitive) {
            ...
            return mb_strtolower(mb_substr($string, 0, $bytes, '8bit'), self::$default_charset) === mb_strtolower($with, self::$default_charset);
        }
    }
    ...
    public static function endsWith($string, $with, $caseSensitive = true)
    {
        ...
        if ($caseSensitive) {
            ...
            return mb_strtolower(mb_substr($string, -$bytes, null, '8bit'), self::$default_charset) === mb_strtolower($with, self::$default_charset);
        }
    }
