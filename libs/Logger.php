...
class Logger {
    ...
    public function log($message, $level, $cate = 'app', $trace=true) {
        ...
        if ($this->traceLevel > 0 && $trace) {
            $count = 0;
            // $ts = debug_backtrace();
            // 使用 DEBUG_BACKTRACE_IGNORE_ARGS 参数会省略函数调用的参数，可以节省一些内存
            $ts = defined('DEBUG_BACKTRACE_IGNORE_ARGS') ? debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS) : debug_backtrace(false);
    ...
    protected function formatMessage($msg) {
        ...
        // substr of session
        $sessionId = substr($sessionId, 0, 4) . substr($sessionId, strlen($sessionId)-4, 4);
